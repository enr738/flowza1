  ...
  if (userId) {
    ...
    if (profile) {
      const { data } = await supabase
        .from('orders')
        .select('*, gigs(title), profiles!buyer_id(username)')
      ...
      <div className="flex gap-4 border-b border-border pb-px">
        {statuses.map(s => (
          <button key={s} className={`pb-4 px-2 text-sm font-medium border-b-2 transition-colors ${s === 'Active' ? 'border-primary-blue text-primary-blue' : 'border-transparent text-text-secondary hover:text-white'}`}>
            {s} {s === 'Active' && activeOrders.length > 0 && <span className="ml-1.5 bg-primary-blue/20 text-primary-blue py-0.5 px-2 rounded-full text-xs">{activeOrders.length}</span>}
          </button>
        ))}
      </div>
