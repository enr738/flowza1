...
export default function PersonalOrders({ searchParams }: { searchParams: { success?: string } }) {
      ...
      <div className="flex gap-4 border-b border-border pb-px">
        {statuses.map(s => (
          <button key={s} className={`pb-4 px-2 text-sm font-medium border-b-2 transition-colors ${s === 'Active' ? 'border-primary-purple text-primary-purple' : 'border-transparent text-text-secondary hover:text-white'}`}>
            {s} {s === 'Active' && <span className="ml-1.5 bg-primary-purple/20 text-primary-purple py-0.5 px-2 rounded-full text-xs">{isSuccess ? '3' : '2'}</span>}
          </button>
        ))}
      </div>
        ...
        {isSuccess ? (
          <Card className="p-6">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-white font-semibold text-lg mb-1">I will design a modern minimal logo for your brand</h3>
                <p className="text-text-secondary text-sm">Seller: Sarah J.</p>
              </div>
              <span className="bg-primary-blue/20 text-primary-blue px-3 py-1 rounded-full text-xs font-semibold">Active</span>
            </div>
            <div className="mt-6 flex gap-4 text-sm text-text-secondary">
              <span>Amount: 150 DZD</span>
              <span>•</span>
              <span>Paid via Edahabia</span>
            </div>
          </Card>
        ) : (
